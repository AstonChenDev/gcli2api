...
def get_image_url(base64_data: str, mime_type: str) -> str:
    """获取图片的访问 URL

    如果配置了 COS 对象存储，将图片上传到 COS 并返回 HTTP URL；
    否则返回 data URL（base64 内嵌）。

    这是所有图片输出的统一入口，确保 COS 逻辑集中在一处。
    ...
    Returns:
        图片 URL（COS HTTP URL 或 data URL）
    """
    from src.cos_uploader import upload_base64_image
...
def extract_content_and_reasoning(parts: list) -> tuple:
    ...
    for part in parts:
        ...
        if "inlineData" in part:
            ...
            url = get_image_url(base64_data, mime_type)
            images.append({
                "type": "image_url",
                "image_url": {
                    "url": url
