...
def parse_quota_reset_timestamp(
    ...
    Args:
        ...
        mode: 调用模式；Antigravity 的 RESOURCE_EXHAUSTED 由独立可配置策略处理
    ...
    try:
        ...
        if (
            mode.lower() == "antigravity"
            and error_obj.get("status") == "RESOURCE_EXHAUSTED"
        ):
            return None
        ...
        # 如果是 RESOURCE_EXHAUSTED 错误且消息完全匹配，设置默认10分钟冷却时间
        if (
